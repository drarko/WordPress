...
Version:      1.0.8
Author:       Themify
Description:  Compare 2 images side by side. It requires to use with a Themify theme (framework 2.0.6+) or the Builder plugin (v 1.2.5).
...
class Builder_AB_Image {
	...
	public function enqueue() {
		wp_register_script( 'builder-ab-image', $this->url . 'assets/scripts.js', array( 'jquery' ), $this->version, true );
		wp_localize_script( 'builder-ab-image', 'builderABImage', apply_filters( 'builder_ab_image_script_vars', array(
			'default_offset_pct' => 0.5
		) ) );
		wp_enqueue_style( 'builder-ab-image', $this->url . 'assets/style.css', null, $this->version );
		wp_enqueue_script( 'builder-ab-image' );
	}

	public function admin_enqueue() {
		wp_enqueue_script( 'builder-ab-image' );
		wp_enqueue_style( 'builder-ab-image-admin', $this->url . 'assets/admin.css' );
	}
