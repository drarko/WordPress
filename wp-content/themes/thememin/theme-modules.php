...
/***************************************************************************/
	...
	function themify_header_slider($data=array()){
		 ...
		 * @var string
		 */
		$output = '<p><span class="label">' . __('Slider', 'themify') . '</span> <select name="setting-header_slider_enabled" class="feature_box_enabled_check">';
		...
		$output .=	'	<input type="text" name="setting-header_slider_posts_slides" value="' . themify_get( 'setting-header_slider_posts_slides' ) . '" class="width2" /> ' . __('number of slides', 'themify') . ' 
					</p>
		...
		$output .= '</div>
					...
					<p>
						...
						for($x=1;$x<=7;$x++){
							if ( themify_get( 'setting-header_slider_visible' ) == $x ) {
			...
			$output .=	'</select> <small>' . __('(# of slides visible at the same time)', 'themify') . '</small>
					...
					<span class="label">' . __('Auto Play', 'themify') . '</span>
							...
							foreach($auto_options as $option){
								if ( $option == themify_get( 'setting-header_slider_auto' ) ) {
									$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
								} else {
									$output .= '<option value="'.$option.'">'.$option.'</option>';
								}
			...
			$output .= '
					...
					<span class="label">' . __('Scroll', 'themify') . '</span>
							...
							foreach($scroll_options as $option){
								if ( $option == themify_get( 'setting-header_slider_scroll' ) ) {
									$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
								} else {
									$output .= '<option value="'.$option.'">'.$option.'</option>';
								}
			...
			$output .= '
					...
					<p>
						...
						foreach($speed_options as $name => $val){
							if ( themify_get( 'setting-header_slider_speed' ) == $val ) {
			...
			$output .= '</select>
					...
					<p>
						...
						foreach($wrap_options as $name => $value){
							if ( themify_get( 'setting-header_slider_wrap' ) == $value ) {
	...
	///////////////////////////////////////////
	// Homepage Footer Slider Function
	///////////////////////////////////////////
	function themify_footer_slider($data=array()){
		 ...
		 * @var string
		 */
		$output = '<p><span class="label">' . __('Slider', 'themify') . '</span> <select name="setting-footer_slider_enabled" class="feature_box_enabled_check">';
		/** Iterate through slider options */
		foreach ( $slider_ops as $key => $val ) {
		...
		$output .=	'	<input type="text" name="setting-footer_slider_posts_slides" value="' . themify_get( 'setting-footer_slider_posts_slides' ) . '" class="width2" /> ' . __('number of slides', 'themify') . ' 
					</p>
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						<span class="label">' . __('Display', 'themify') . '</span>
								...
								foreach($display_options as $option){
									if ( $option == themify_get( 'setting-footer_slider_default_display' ) ) {
										$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
									} else {
										$output .= '<option value="'.$option.'">'.$option.'</option>';
									}
			...
			$output .= '<div class="feature_box_images" '.$display_images_display.'>';
					...
					foreach ( $image_options as $option ) {
						...
						<div class="uploader-fields">
							...
							<span class="label">' . __('Image', 'themify') . ' '.$x.'</span>
							<input type="text" class="width10 feature_box_img" id="setting-slider_images_'.$option.'_image" name="setting-footer_slider_images_'.$option.'_image" value="' . themify_get( 'setting-footer_slider_images_'.$option.'_image' ) . '" /> 
		...
		$output .= '<input type="hidden" name="setting-default_post_layout" class="val" value="'.$val.'" />
					...
					<p>
						...
						foreach ( $default_layout_options as $layout_option ) {
							if ( $layout_option['value'] == themify_get( 'setting-default_layout_display' ) ) {
...
<span class="pushlabel vertical-grouped"><label for="setting-excerpt_more"><input type="checkbox" value="1" id="setting-excerpt_more" name="setting-excerpt_more" '.checked( themify_get( 'setting-excerpt_more' ), 1, false ).'/> ' . __('Display more link button in excerpt mode as well.', 'themify') . '</label></span>
		...
		$output .=	'<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_post_title' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_unlink_post_title' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_post_meta' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_post_date' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_post_image' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_unlink_post_image' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
	...
	function themify_default_post_layout($data=array()){
		...
		$output .= '</p>
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_page_post_title' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_page_unlink_post_title' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_page_post_meta' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
					...
					<p>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_page_post_date' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
		...
		$output .=	'</select>
						...
						foreach ( $default_options as $title_option ) {
							if ( $title_option['value'] == themify_get( 'setting-default_page_unlink_post_image' ) ) {
								$output .= '<option selected="selected" value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$title_option['value'].'">'.$title_option['name'].'</option>';
							}
