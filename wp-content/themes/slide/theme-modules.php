	...
	function themify_homepage_featurebox($data=array()){
		...
		$output = '<p>
						...
						foreach ( $effect_options as $name => $val ) {
							$output .= '<option value="'.$val.'" ' . selected( themify_get( 'setting-feature_box_effect' ), isset( $data['setting-feature_box_effect'] ) ? $val: 'slide', false ) . '>'.$name.'</option>';
						}
			$output .=	'</select>
					</p>
					<p>
						...
						foreach ( $speed_options as $name => $val ) {
							$output .= '<option value="'.$val.'" ' . selected( themify_get( 'setting-feature_box_speed' ), isset( $data['setting-feature_box_speed'] ) ? $val: 1000, false ) . '>'.$name.'</option>';	
						}
			$output .= '</select>
					</p>
					<p>
						...
						foreach ( $auto_options as $name => $val ) {
							$output .= '<option value="'.$val.'" ' . selected( themify_get( 'setting-feature_box_auto' ), isset( $data['setting-feature_box_auto'] ) ? $val: 4000, false ) . '>'.$name.'</option>';	
						}
