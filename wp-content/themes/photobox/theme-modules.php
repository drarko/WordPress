...
/***************************************************************************/
	...
	function themify_homepage_featurebox($data=array()){
		...
		if ( 'images' == themify_get( 'setting-feature_box_display' ) ) {
		...
		$effect_options = array(__('Fade', 'themify') => 'fade', __('Slide Horizontal', 'themify') => 'scrollHorz', __('Slide Vertical', 'themify') => 'scrollVert');
		$speed_options = array( __('Normal', 'themify')=>1000, __('Slow', 'themify')=>2000, __('Fast', 'themify')=>500);
		 ...
		 * @var string
		 */
		$output = '<p><span class="label">' . __('Enable Slider', 'themify') . '</span> <select name="setting-feature_box_enabled" class="feature_box_enabled_check">';
		/** Iterate through slider options */
		foreach ( $slider_ops as $key => $val ) {
			$output .= '<option value="'.$val.'" ' . selected( themify_get( 'setting-feature_box_enabled' ), $val, false ) . '>' . $key . '</option>';
		}
		$output .= '</select>' . '</p>
		
					<div class="feature_box_enabled_display" '.$enabled_display.'>
					<p><span class="label">' . __('Display', 'themify') . '</span> <input type="radio" class="feature-box-display" name="setting-feature_box_display" value="posts" '.$display_posts.' /> ' . __('Posts', 'themify') . ' <input type="radio" class="feature-box-display" name="setting-feature_box_display" value="images" '.$display_images.' /> ' . __('Images', 'themify') . '</p>
					<p class="pushlabel feature_box_posts" '.$display_posts_display.'>';
							$output .= wp_dropdown_categories(array("show_option_all"=>__('All Categories', 'themify'),"hide_empty"=>0,"echo"=>0,"name"=>"setting-feature_box_posts_category","selected"=> themify_get( 'setting-feature_box_posts_category' )));
		$output .=	'	<input type="text" name="setting-feature_box_posts_slides" value="' . themify_get( 'setting-feature_box_posts_slides' ) . '" class="width2" /> ' . __('number of slides', 'themify') . ' 
					</p>
					<div class="feature_box_images" '.$display_images_display.'>
						';
						
						foreach ( $image_options as $option => $x ){
		$output .= '		<div class="uploader-fields">
								<span class="label">' . __('Title', 'themify') . ' '.$x.'</span>
								<input type="text" class="width10" name="setting-feature_box_images_'.$option.'_title" value="' . themify_get( 'setting-feature_box_images_'.$option.'_title' ) . '" />
								<span class="label">' . __('Link', 'themify') . ' '.$x.'</span>
								<input type="text" class="width10" name="setting-feature_box_images_'.$option.'_link" value="' . themify_get( 'setting-feature_box_images_'.$option.'_link' ) . '" />
								<span class="label">' . __('Image', 'themify') . ' '.$x.'</span>
								<input type="text" class="width10 feature_box_img" id="setting-feature_box_images_'.$option.'_image" name="setting-feature_box_images_'.$option.'_image" value="' . themify_get( 'setting-feature_box_images_'.$option.'_image' ) . '" />
								<div class="pushlabel" style="display:block;">' . themify_get_uploader('setting-feature_box_images_'.$option.'_image', array('tomedia' => true)) . '</div>
							</div>';
						}
			...
			$output .= '</select>
					...
					<p>
						<span class="label">' . __('Timeout', 'themify') . '</span> 
			...
			$output .=	'</select> <small>' . __('(Off = disable autoplay)', 'themify') . '</small>
					...
					<span class="label">' . __('Image Size', 'themify') . '</span>  
						<input type="text" class="width2" name="setting-feature_image_width" value="' . themify_get( 'setting-feature_image_width' ) . '" /> ' . __('width', 'themify') . ' <small>(px)</small>  
						<input type="text" class="width2" name="setting-feature_image_height" value="' . themify_get( 'setting-feature_image_height' ) . '" /> ' . __('height', 'themify') . ' <small>(px)</small>
					</p>
					</div>';		
		return $output;
	}
	...
	function themify_homepage_featurebox_action($data=array()){
		...
		jQuery(document).ready(function($){
			$('#slider .slides').after('<div class="slide-nav-wrap"> <a class="next-prev-slide-button prev-slide" href="#">&lt;</a> <a class="next-prev-slide-button next-slide" href="#">&gt;</a> </div>').cycle({
				fx:     '<?php echo '' != $data['setting-feature_box_effect']? $data['setting-feature_box_effect']: 'fade'; ?>', 
                speed:   <?php echo '' != $data['setting-feature_box_speed']? $data['setting-feature_box_speed']: '1000'; ?>, 
                timeout: <?php echo '' != $data['setting-feature_box_auto']? $data['setting-feature_box_auto']: 0; ?>,
				pause: 1,
				pager: '#slider .slider-nav',
			...
			//pause on click
			$('.slide-nav-wrap a, .slider-nav a').click(function(){
