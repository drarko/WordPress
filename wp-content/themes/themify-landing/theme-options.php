...
class Themify {
	...
	function template_redirect() {
		...
		elseif ( is_single() ) {
			...
			if ( is_singular( 'portfolio' ) ) {
				...
				$this->hide_date = (themify_get('hide_post_date') != 'default' && themify_check('hide_post_date')) ? themify_get('hide_post_date') : themify_get('setting-default_portfolio_single_post_date');
                        ...
                        elseif(is_singular('event')){
                            $post_image_height = themify_get( 'image_width' );
                            $post_image_height = themify_get( 'image_height' );
                            $default_width = self::$single_event_image_width;
                            $default_height = self::$single_event_image_height;
                        }
                        else{
			...
			$this->width = $post_image_width>=0?$post_image_width:$default_width;
                        $this->height = $post_image_height>=0?$post_image_height:$default_height;
