...
if('' == themify_get('setting-header_slider_enabled') || 'on' == themify_get('setting-header_slider_enabled')) { ?>
	...
	<div id="header-slider" class="slider">
		...
		<?php if(get_cat_name(themify_get('setting-header_slider_posts_category')) != ""){ ?>
			<h3 class="slider-title"><?php echo get_cat_name(themify_get('setting-header_slider_posts_category')); ?></h3>
		<?php } else { ?>
		...
		<ul class="slides clearfix" data-slider="<?php esc_attr_e( base64_encode( json_encode( $slider_args ) ) ); ?>">
			...
			$img_width = themify_check('setting-header_slider_width')?	themify_get('setting-header_slider_width'): '170';
			$img_height = themify_check('setting-header_slider_height')? themify_get('setting-header_slider_height'): '120';

			if(themify_get('setting-header_slider_posts_category') != ""){
				$cat = "&cat=".themify_get('setting-header_slider_posts_category');
			} else {
				$cat = "";
			}
			if(themify_get('setting-header_slider_posts_slides') != "" ){
				$num_posts = "showposts=".themify_get('setting-header_slider_posts_slides')."&";
			} else {
				$num_posts = "showposts=7&";
			}
			if(themify_get('setting-header_slider_display') == "images"){
