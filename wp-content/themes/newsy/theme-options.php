...
class Themify {
	...
	static $single_image_width = 968;
	static $single_image_height = 500;
	
	// Grid4
	static $grid4_width = 212;
	static $grid4_height = 140;
	
	// Grid3
	static $grid3_width = 296;
	static $grid3_height = 190;
	
	// Grid2
	static $grid2_width = 464;
	...
	function template_redirect() {
		...
		elseif( is_single() ) {
			...
			$this->width =$post_image_width>=0?$post_image_width:self::$single_image_width;
                ...
                $is_page = is_page();
                if($this->layout === 'sidebar1' || ($is_page && $this->layout == 'sidebar1 sidebar-left')){
                    $sidebar = self::$sidebar1_content_width;
                }
                elseif($this->layout === 'sidebar2' || ($is_page && ($this->layout == "sidebar2 content-left" || $this->layout == "sidebar2 content-right"))){
                    $sidebar = self::$sidebar2_content_width;
                }
                else{
                    $sidebar = false;
                }
                 if($sidebar && empty($this->width) &&  empty($this->height) && $this->height!=='0') {
                    $ratio = $this->width / self::$content_width;
                    $aspect = $this->width == 0 ? 0 : $this->height / $this->width;
                    $this->width = round($ratio * $sidebar);
