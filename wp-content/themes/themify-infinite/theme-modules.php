...
function themify_drop_cap_module( $data = array() ) {
	...
	$html = sprintf(
		...
		__( 'Disable Drop Cap styling that appears in the first paragraph of post content/excerpt', 'themify' )
	);
	return $html;
}
...
/**
 * Choose pagination or infinite scroll for single view.
 *
 * @param array $data
 *
 * @return string
 */
function themify_pagination_infinite_single( $data = array() ) {
	$html = '<p>';

	//Infinite Scroll
	$html .= '<input ' . checked( themify_check( 'setting-single_more_posts' ) ? themify_get( 'setting-single_more_posts' ) : 'infinite', 'infinite', false ) . ' type="radio" name="setting-single_more_posts" value="infinite" /> ';
	$html .= __( 'Infinite Scroll (the previous post will load automatically when scrolling end of the post)', 'themify' );
	$html .= '<br/>';

	$html .= '<span class="indent">';
	$html .= '<label for="setting-single_autoinfinite"><input type="checkbox" id="setting-single_autoinfinite" name="setting-single_autoinfinite"
' . checked( themify_check( 'setting-single_autoinfinite' ), true, false ) . '/> ' . __( 'Disable automatic infinite scroll (load more button will show)', 'themify' ) . '</label>';
	$html .= '</span>';
	$html .= '<br/>';

	// Ad code
	$html .= '<span class="indent">';
	$html .= '<label for="setting-single_ad_code" class="clearfix">' . esc_html__( 'Display an ad code in between each post load:' ) . '</label>';

	$html .= '<textarea id="setting-single_ad_code" name="setting-single_ad_code" class="widthfull" rows="7">' . esc_textarea( themify_get( 'setting-single_ad_code' ) ) . '</textarea>';
	$html .= '</span>';
	$html .= '<br/><br/>';

	//Numbered pagination
	$html .= '<input ' . checked( themify_get( 'setting-single_more_posts' ), 'pagination', false ) . ' type="radio" name="setting-single_more_posts" value="pagination" /> ';
...
function themify_pagination_infinite( $data = array() ) {
	...
	$html .= __( 'Infinite Scroll (posts will load automatically when scrolling to the bottom of the page)', 'themify' );
	$html .= '<br/>';

	$html .= '<span class="indent">';
	$html .= '<label for="setting-autoinfinite"><input type="checkbox" id="setting-autoinfinite" name="setting-autoinfinite" ' . checked( themify_check( 'setting-autoinfinite' ), true, false ) . '/>
' . __( 'Disable automatic infinite scroll (load more button will show)', 'themify' ) . '</label>';
	$html .= '</span>';

	// Number of posts betwen ad code
	$number = '<input type="number" name="setting-ad_code_cycle" value="' . esc_attr( themify_check( 'setting-ad_code_cycle' ) && is_numeric( themify_get( 'setting-ad_code_cycle' ) ) ? themify_get( 'setting-ad_code_cycle' ) : 3 ) . '" class="width2">';

	// Ad code
	$html .= '<span class="indent">';
	$html .= '<label for="setting-ad_code" class="clearfix">' . sprintf( __( 'Display an ad code after %s posts:' ), $number ) . '</label>';

	$html .= '<textarea id="setting-ad_code" name="setting-ad_code" class="widthfull" rows="7">' . esc_textarea( themify_get( 'setting-ad_code' ) ) . '</textarea>';
	$html .= '<br/><br/>';
	$html .= '</span>';
