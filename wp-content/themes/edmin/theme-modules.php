		...
		$output .=	'	<input type="text" name="setting-header_slider_posts_slides" value="' . themify_get( 'setting-header_slider_posts_slides' ) . '" class="width2" /> ' . __('number of slides', 'themify') . ' 
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						...
						foreach($display_options as $name => $value){
							$output .= '<option value="'.$value.'" '.selected( themify_check( 'setting-header_slider_default_display' )? themify_get( 'setting-header_slider_default_display' ) : 'none', $value, false).'>'.$name.'</option>';
						}
			$output .= '</select>
					</p>';
