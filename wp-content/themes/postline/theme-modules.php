...
/***************************************************************************/
	...
	function themify_pagination_infinite($data=array()){
		...
		//Infinite Scroll
		$html .= '<label><input ' . checked( themify_check( 'setting-more_posts' ) ? themify_get( 'setting-more_posts' ) : 'infinite', 'infinite', false ) . ' type="radio" name="setting-more_posts" value="infinite" /> ';
		$html .= __('Infinite Scroll (posts are loaded on the same page)', 'themify') . '</label>';
		$html .= '<br/>';
		...
		//Numbered pagination
		$html .= '<label><input ' . checked( themify_get( 'setting-more_posts' ), 'pagination', false ) . ' type="radio" name="setting-more_posts" value="pagination" /> ';
		$html .= __('Standard Pagination', 'themify') . '</label>';
		$html .= '</p>';
	...
	function themify_default_layout($data=array()){
		...
		$timeline_options = array(
			array('name'=>__('Monthly', 'themify'),'value'=>'monthly'),
		...
		$output .= '<input type="hidden" name="setting-default_post_layout" class="val" value="'.$val.'" />
					...
					<p>
						<span class="label">' . __('Timeline Query', 'themify') . '</span> 
						<select name="setting-default_timeline_query">';
						foreach ( $timeline_options as $timeline_option ) {
							if ( $timeline_option['value'] == themify_get( 'setting-default_timeline_query' ) ) {
								$output .= '<option selected="selected" value="'.$timeline_option['value'].'">'.$timeline_option['name'].'</option>';
							} else {
								$output .= '<option value="'.$timeline_option['value'].'">'.$timeline_option['name'].'</option>';
							}
						}
		$output .=	'	</select>
					</p>
