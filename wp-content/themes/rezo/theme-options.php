...
class Themify {
	...
	public $menu_category_list = '';
	public $link_item = false;
	public $zoom_image = false;
	public $menu_layout = '';
	...
	static $content_width = 896;
	static $sidebar1_content_width = 584;
	
	// Default Single Post Layout
	static $single_content_width = 896;
	static $single_sidebar1_content_width = 584;
	
	// Default Single Image Size
	static $single_image_width = 896;
	static $single_image_height = 400;
	
	// Grid4
	static $grid4_width = 196;
	static $grid4_height = 140;
	
	// Grid3
	static $grid3_width = 274;
	static $grid3_height = 180;
	
	// Grid2
	static $grid2_width = 430;
	static $grid2_height = 260;
	
	// List Large
	static $list_large_image_width = 640;
	static $list_large_image_height = 400;
	 
	// List Thumb
	static $list_thumb_image_width = 140;
	...
	// List Post
	static $list_post_width = 896;
	static $list_post_height = 380;
	...
	function template_redirect() {
		...
		if( is_page() ) {
			...
			// Rezo specific
			$this->menu_category = themify_get('menu_category');
			$this->menu_category_list = themify_get('menu_category_list');
			$this->link_item = themify_get('link_to_menu')? true : false;
			$this->zoom_image = 'no' == themify_get('zoom_image')? false : true;
			$this->menu_layout = themify_get('menu_layout');
		}
		elseif( is_single() ) {
	 ...
	 * @return string
	 */
	function external_link($meta = 'external_link'){
	...
	function get_category_description(){
		...
		if ( !empty( $category_description ) ){
			return '<span class="category-description">' . $category_description . '</span>';
		}
	}
	...
	/**
	 * Perform adjustments for menu template
	 * 
	 */
	function calculate_menu_layout(){
		if(themify_get("menu_layout") == "col3-1"){ 
			$this->width = 274;
			$this->height = 180;
		} else if(themify_get("menu_layout") == "fullwidth"){
			$this->width = 620;
			$this->height = 400;
		} else {
			$this->width = 95;
		...
		if(themify_check("menu_image_width")){
			$this->width = themify_get("menu_image_width");
		}
		if(themify_check("menu_image_height")){
			$this->height = themify_get("menu_image_height");
		}
	}
	 ...
	 * @return string
	 */
	function menu_loop($cats, $default = true){
		...
		if($default) {
			...
			foreach($cats as $cat){
				...
				echo '<div class="category-section">';
					...
					query_posts('post_type=menu&showposts=-1&menu_categories='.$cat->category_nicename);
					
					echo '<h3 class="menu-category">' . $cat->cat_name . '</h3>';
					echo $this->get_category_description();
					while (have_posts()){
						the_post();
						get_template_part('includes/loop-menu');
					}
			...
			query_posts(array('post_type'=>'menu','menu_category'=>$cats));
