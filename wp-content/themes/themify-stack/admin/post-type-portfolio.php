...
function themify_theme_portfolio_meta_box( $args = array() ) {
	...
	return array(
		...
		array(
			...
			'description' => __( 'This layout arrangement applies in portfolio archive if masonry style is selected.', 'themify' ),
			'type'        => 'layout',
			'show_title'  => true,
			'meta'        => array(
				array(
					...
					'title'    => __( 'Large tile, Image on the left', 'themify' ),
					'selected' => true,
				),
				array(
					'value' => 'size-small image-left',
					'img'   => 'images/layout-icons/small-image-left.png',
					'title' => __( 'Small tile, Image on the left', 'themify' ),
				),
				array(
					'value'    => 'size-large image-right',
					'img'      => 'images/layout-icons/large-image-right.png',
					'title'    => __( 'Large tile, Image on the right', 'themify' ),
					'selected' => true,
				),
				array(
					'value' => 'size-small image-right',
					'img'   => 'images/layout-icons/small-image-right.png',
					'title' => __( 'Small tile, Image on the right', 'themify' ),
				),
			)
		),
		// Post Image
        ...
        array(
            ...
            'description' => __( 'This is the masonry gallery in the portfolio single view', 'themify' ),
		...
		array(
		    ...
		    'type'        => 'separator',
		    'meta'        => array(
		        'html' => '<h4>' . __( 'Portfolio Style in Archive Views', 'themify' ) . '</h4><hr class="meta_fields_separator"/>'
		    ),
		),
		// Background Color
		array(
	...
	class Themify_Portfolio {
		...
		function section_link( $more_link = false, $more_text, $post_type ) {
			if ( $more_link ) {
				...
				return '<p class="more-link-wrap"><a href="' . esc_url( $more_link ) . '" class="more-link">' . esc_html( $more_text ) . '</a></p>';
			}
			return '';
		}
		...
		function shortcode($atts = array(), $post_type){
			...
			if( $posts ) {
				...
				$out .= '<div class="loops-wrapper shortcode ' . esc_attr( $post_type  . ' ' . $style . ' '. $cpt_layout_class ) .'">';
...
if ( ! function_exists( 'themify_default_portfolio_single_layout' ) ) {
	...
	function themify_default_portfolio_single_layout( $data=array() ) {
		...
		$output .= '<p>
						...
						$output .= '<input type="hidden" name="' . esc_attr( $prefix ) . 'layout" class="val" value="' . esc_attr( $layout ) . '" />';
		$output .= '</p>';
		...
		$output .= '<p>
						...
						<select name="' . esc_attr( $prefix . 'post_image' ) . '">' . themify_options_module( $default_options, $prefix . 'post_image', true, 'yes' ) . '
						</select>
					</p>';
		...
		$output .= '
			<p>
				...
				<label for="' . esc_attr( $prefix ) . 'disable">
					<input type="checkbox" id="' . esc_attr( $prefix . 'disable' ) . '" name="' . esc_attr( $prefix . 'disable' ) . '" '. checked( themify_get( $prefix.'disable' ), 'on', false ) .'/> ' . __('Remove Portfolio Navigation', 'themify') . '
				</label>
				<span class="pushlabel vertical-grouped">
				<label for="' . esc_attr( $prefix ) . 'same_cat">
					<input type="checkbox" id="' . esc_attr( $prefix . 'same_cat' ) . '" name="' . esc_attr( $prefix . 'same_cat' ) . '" '. checked( themify_get( $prefix. 'same_cat' ), 'on', false ) .'/> ' . __('Show only portfolios in the same category', 'themify') . '
				</label>
				</span>
			</p>';
	...
	function themify_default_portfolio_index_layout( $data=array() ) {
		...
		$output .= '<p>
						...
						foreach($post_layout_options as $option){
							...
							$output .= '<a href="#" class="preview-icon ' . esc_attr( $class ) . '" title="' . esc_attr( $option['title'] ) . '"><img src="' . esc_attr( THEME_URI.'/'.$option['img'] ) . '" alt="' . esc_attr( $option['value'] ) . '"  /></a>';
						}
		...
		$output .=	'<p>
						...
						<br/>
						<small>' . __( 'Masonry produces the post stacking layout (portfolios are placed above each other)', 'themify' ) . '</small>
					</p>';
	...
	function themify_portfolio_slug($data=array()){
		...
		return '
			<p>
				...
				<input type="text" name="themify_portfolio_slug" value="' . esc_attr( $portfolio_slug ) . '" class="slug-rewrite">
				<br />
