...
class Themify {
	...
	// Index Portfolio
	static $index_portfolio_image_width = 480;
	static $index_portfolio_image_height = 480;
	...
	private $default_sidebar = 'sidebar-none';
	private $default_layout = 'grid3';
	...
	function __construct() {
		...
		if($this->layout == '' ) $this->layout = $this->default_sidebar;
		
		$this->post_layout = themify_get('setting-default_post_layout');
		if($this->post_layout == '') $this->post_layout = $this->default_layout;
	...
	function template_redirect() {
		...
		if( is_page() ) {
			...
			if($this->layout == ''){
				$this->layout = $this->default_sidebar;
                        }
			...
			if($this->post_layout == ''){
				$this->post_layout = $this->default_layout;
                        }
		...
		elseif (is_tax('portfolio-category')) {
			...
			$this->hide_image = themify_check('setting-default_portfolio_index_post_image')?
					themify_get('setting-default_portfolio_index_post_image') : 'no';

			$this->hide_image = themify_check('setting-default_portfolio_index_unlink_post_image')?
					themify_get('setting-default_portfolio_index_unlink_post_image') : 'no';
		...
		elseif( is_singular('post') || is_singular('portfolio') ) {
			...
			if( !$is_portfolio){
				...
				$this->hide_image = (themify_get('hide_post_image') != 'default' && themify_check('hide_post_image')) ? themify_check( 'setting-default_portfolio_single_post_image' ) ? themify_get( 'setting-default_portfolio_single_post_image' ) : 'no' : 'no';

				$this->unlink_image = (themify_get('unlink_post_image') != 'default' && themify_check('unlink_post_image')) ? themify_check( 'setting-default_portfolio_single_unlink_post_image' ) ? themify_get( 'setting-default_portfolio_single_unlink_post_image' ) : 'no' : 'no';
		...
		if ( ! is_singular() ) {
			if(empty($this->width) &&  empty($this->height) && $this->height!=='0'  && ($this->layout === 'sidebar1' || $this->layout === 'sidebar1 sidebar-left')) {
				...
				if($this->height>0 && $this->width>0 &&  $this->width>0){
