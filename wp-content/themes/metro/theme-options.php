...
class Themify {
	...
	static $content_width = 998;
	static $sidebar1_content_width = 670;
	
	// Default Single Post Layout
	static $single_content_width = 998;
	static $single_sidebar1_content_width = 670;
	
	// Default Single Image Size
	static $single_image_width = 998;
	static $single_image_height = 500;
	
	// Grid3
	static $grid3_width = 670;
	static $grid3_height = 300;
	
	// Grid2
	static $grid2_width = 670;
	static $grid2_height = 300;
	
	// List Post
	static $list_post_width = 998;
	static $list_post_height = 0;
	
	// Index Portfolio
	static $index_portfolio_image_width = 998;
	static $index_portfolio_image_height = 494;
	...
	function template_redirect() {
		...
		elseif( is_tax('portfolio-category') ){
			$pcpre = 'setting-default_portfolio_index_';
			$this->hide_title = themify_get($pcpre.'title')? themify_get('setting-default_portfolio_index_title') : '';
			
			$this->unlink_title = themify_get($pcpre.'unlink_post_title')? themify_get($pcpre.'unlink_post_title') : '';
			
			$this->hide_date = themify_get($pcpre.'post_date')? themify_get($pcpre.'post_date') : '';
			
			$this->hide_meta = themify_get($pcpre.'post_meta')?themify_get($pcpre.'post_meta') : '';
			
			$this->display_content = themify_get($pcpre.'display')?themify_get($pcpre.'display') : 'none';
			
			$this->layout = themify_get($pcpre.'layout');
			if($this->layout == '')	{
                            $this->layout = 'sidebar-none'; 
                        }
			$this->post_layout = themify_get($pcpre.'post_layout');
			if($this->post_layout == ''){
                            $this->post_layout = 'grid3';
                        }
				
			$this->width = themify_check($pcpre.'image_post_width') ?
				themify_get($pcpre.'image_post_width'):	self::$index_portfolio_image_width;
			$this->height = themify_get($pcpre.'image_post_height') ?
				themify_get($pcpre.'image_post_height'): self::$index_portfolio_image_height;
		}
	...
	public function get_post_color_class($post_id){
		$post_color_class = (themify_check('post_color') && themify_get('post_color') != 'default') ? themify_get('post_color') : 'default';
		
		if('default' == $post_color_class){
			$post_color_class = themify_get('setting-default_color')? themify_get('setting-default_color') : 'default';
		}
		return $post_color_class;
	}
	...
	/**
	 * Outputs several classes for layout like colors, size and others
	 * @param bool Whether to echo or not
	 * @return string
	 */
	function extra_classes($echo = true){
		$classes = themify_check('_portfolio_color')? themify_get('_portfolio_color') : 'default';
		if('default' == $classes){
			$classes = themify_get('setting-default_portfolio_color')? themify_get('setting-default_portfolio_color') : 'default';
		}
		if($echo){
			echo $classes;
		} else {
