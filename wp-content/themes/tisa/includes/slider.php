	...
	<div id="slider-inner" class="pagewidth">
		...
		<div id="slider" class="slider">
			...
			<?php
				...
				$img_width = themify_check('setting-slider_width')?	themify_get('setting-slider_width'): '978';
				...
				if(themify_check('setting-slider_display') && themify_get('setting-slider_display') == "images"){ 
						...
						if(themify_check($option.'_image')){
							echo '<li>';
								...
								if(themify_check($option.'_link')) {
									...
									echo "<div class='slide-feature-image'><a href='$link' $title_attr>" . themify_get_image("src=$image&ignore=true&w=$img_width&h=$img_height&alt=$alt") . '</a></div>';
									
								} else {
									echo $title? '<div class="slide-content-wrap"><div class="slide-content-wrap-inner"><h3 class="slide-post-title">'.$title.'</h3></div></div>' : '';
									
									echo '<div class="slide-feature-image">' . themify_get_image("src=$image&ignore=true&w=$img_width&h=$img_height&alt=$alt") . '</div>';
								}
							echo '</li>';
						}
					}
					...
					if( have_posts() ) {
						...
						<li id="slider-<?php the_ID(); ?>" class="type-slider">
							...
							<div class="slide-feature-image">
								<a href="<?php echo $link; ?>" title="<?php the_title_attribute(); ?>">
									<?php
									themify_image("ignore=true&w=$img_width&h=$img_height"); ?>
								</a>	
							</div>
							<!-- /.slide-feature-image -->
						</li>
