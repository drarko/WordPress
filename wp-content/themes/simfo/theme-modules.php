...
/***************************************************************************/
		...
		$output .= '</div>
				...
				<span class="label">' . __('Auto Play', 'themify') . '</span>
						...
						foreach($auto_options as $option){
							if ( $option == themify_get( 'setting-slider_auto' ) ) {
		...
		$output .= '
				...
				<p>
					...
					foreach($speed_options as $name => $val){
						if ( $val == themify_get( 'setting-slider_speed' ) ) {
		...
		$output .= '</select>
				...
				<p>
					...
					foreach($effect_options as $effc => $val){
						if ( $val == themify_get( 'setting-slider_effect' ) ) {
	...
	function themify_slider_action($data=array()){
		if(themify_get( 'setting-slider_speed' ) == ''){
			$speed = "4000";	
		} else {
			$speed = themify_get( 'setting-slider_speed' );
		}
		if(themify_get( 'setting-slider_auto' ) == ''){
			$auto = "yes";	
		} else {
			$auto = themify_get( 'setting-slider_auto' );
		}
		if(themify_get( 'setting-slider_effect' ) == ''){
			$effc = 'slide';
		} else {
			$effc = themify_get( 'setting-slider_effect' );
		}		
		?>
		<!-- slider -->
		<script>
...
<span class="pushlabel vertical-grouped"><label for="setting-excerpt_more"><input type="checkbox" value="1" id="setting-excerpt_more" name="setting-excerpt_more" '.checked( themify_get( 'setting-excerpt_more' ), 1, false ).'/> ' . __('Display more link button in excerpt mode as well.', 'themify') . '</label></span>
	...
	function themify_footer_logo($data=array()){
		if( isset( $data['attr'] ) && isset( $data['attr']['target'] ) && $data['attr']['target'] != ''){
			$target = "<span class='hide target'>".$data['attr']['target']."</span>";
		} else {
			$target = '';
		}
		if ( themify_get( 'setting-footer_logo' ) == 'image' ) {
			$text = '';
		...
		return '<div class="themify_field_row">
					...
					<div class="uploader-fields pushlabel image" '.$image_display.'>
						<input type="text" class="width10" id="setting-footer_logo_image_value" name="setting-footer_logo_image_value" value="'.themify_get( 'setting-footer_logo_image_value' ).'" /> <br />
					...
					<p class="pushlabel image" '.$image_display.'>
						<input type="text" name="setting-footer_logo_width" class="width2" value="'. themify_get( 'setting-footer_logo_width' ).'" /> ' . __('width', 'themify') . '
						<input type="text" name="setting-footer_logo_height" class="width2" value="'. themify_get('setting-footer_logo_height' ).'" /> ' . __('height', 'themify') . '
					</p>
				</div>';	
	}
